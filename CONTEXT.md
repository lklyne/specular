...
- **Idle** — the app is unfocused, the blur grace has elapsed, no awake hold is outstanding, and no agent activity is in its trailing window. Idle pages are frozen (`Page.setWebLifecycleState`), never hidden: a frozen page still yields its last frame to `capturePage`, which is how agents see (`page-idle-throttle.ts`). Not CPU-throttled — see the ADR's postmortem.
