...
- **Durable Object per canvas** — the Cloudflare unit that runs the Yjs sync server (`y-partyserver`'s `YServer`, WebSocket Hibernation) and persists the doc via its `onLoad`/`onSave` hooks into DO storage (persistence is our wiring, not automatic). The data plane: a canvas lives in the cloud whether or not a desktop app is connected. Code lives in the `server/` workspace package.
...
- **Share popover** — the toolbar-anchored surface for sharing (ADR 0018 §4b): Copy link + scope dropdown, active-link list with reset/revoke. The first Copy link is the publish moment — it attaches the canvas to the owner principal's cloud set and pushes doc + assets; an unshared canvas has zero server footprint. Links always open the view+comment web client; `specular://` hands off to the desktop app.
