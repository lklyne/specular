...
- Region anchor → dashed rose-400 rectangle, always visible while on-document (filtered by `status` and the document-binding gate). Click opens the thread. Region rects are in canvas coords; page-anchored regions translate with page drags/nudges (and will scroll-follow when scroll tracking lands), canvas-anchored regions stay put.
...
- **Anchored items travel with their page.** Drag/nudge id sets expand to include anchored entities; page-anchored region annotations translate by the page's applied delta inside the same gesture (`translateAnnotationsAnchoredToPage`, `page-anchor-state.ts`) — one undo step either way. Positions stay in canvas coordinates (no stored offset).
...
- Anchors persist transparently: as a `pageAnchor` field on the JSON Canvas node (`specular` extension block for text nodes) and on the annotation record. Scroll tracking is a deliberate non-feature for now — see the ADR's follow-ups.
