...
The generic "hook to a page" utility ([ADR 0029](./docs/adr/0029-page-anchored-entities.md), `src/shared/page-anchor.ts`). A canvas item is either **free-form / canvas-bound** (default) or carries a `PageAnchor { pageId, pageUrl? }`. Consumers today: `text` (stickies), `drawing`, and annotations (§Annotations).
...
- **Anchored items travel with their page.** Drag/nudge id sets expand to include anchored entities; those positions stay in canvas coordinates (no stored offset). A **page-anchored region annotation** instead stores its rect in the page's *document* space (`anchor.docRect`, page CSS px), so it travels with page drags **and scroll-follows** for free — the transform moves it; nothing translates the anchor. `canvasRectToPageDocRect` / `regionCanvasRect` (`page-anchor-state.ts`) convert between the marquee's canvas rect and the stored docRect. A grab-less region keeps `canvasRect` and never moves with pages.
...
- Anchors persist transparently: as a `pageAnchor` field on the JSON Canvas node (`specular` extension block for text nodes) and on the annotation record. Region annotations scroll-follow via the `docRect` anchor (above); entities and other anchor kinds stay pinned to the page frame (do not scroll-follow) — see the ADR's follow-ups.
