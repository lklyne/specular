...
- **Tab** — one canvas open in the workspace: `{id, name, snapshot, annotations}` in `workspaceTabs`. Exactly one tab is active, and the runtime arrays hold only that tab's state; the others are inert snapshots plus a `.canvas` file each. `specular workspace` reports the active tab and every tab (id, name, entityCount) in its `appState`, so an agent can tell which canvas answered.
- **Tab ref** — how a caller outside the app names a tab: an id or an exact name. `resolveWorkspaceTabRef` (`src/main/runtime/workspace-tab-refs.ts`) is the single resolver — id first, then exact name; an ambiguous name errors with the matching ids and an unknown ref errors with the available tabs. It never guesses. `specular tab new <name>` creates a tab without activating it (and refuses a duplicate name, since `.canvas` filenames derive from the name); `specular tab switch <ref>` is the only CLI verb that moves the user's focus.
- **Tab context** — a scoped, synchronous swap that lets a write target a background tab without moving the user's focus: `withTabContext(tabId, fn)` (`src/main/runtime/workspace-tab-context.ts`) hydrates the target's snapshot into the runtime arrays and a fresh detached `Y.Doc`, runs the normal `applyCanvasPatch` spine, folds the result back into the tab's record and `.canvas` file, then restores the user's canvas. One JS turn, persistence suspended throughout — so the autosave debounce and IPC handlers can never observe the swap. `--tab <ref>` on any verb selects it (resolved once, server-side, via **Tab ref**); reads of a background tab serialize its record with no swap at all. Two consequences by design: pages are refused on a background tab (they need a live `WebContentsView`; the target's existing pages ride through as data only), and the write never reaches the user's undo stack — Cmd+Z after an agent write is a no-op with respect to it.
...
The target architecture is [ADR 0018](./docs/adr/0018-cloud-sync-and-canvas-sharing.md). The spike (`docs/plans/cloud-sync-spike.md`) has landed the substrate behind the `debug.cloudShare` preference flag — server package, desktop transport, share popover, `specular connect` CLI verb — but ships to no one by default: the local-first disk path (`.canvas` files) remains the only shipping mode until the auth layer and a real web client are scheduled. Terms recorded so planning shares vocabulary:
...
- **Durable Object per canvas (ADR target) / per workspace (spike)** — the Cloudflare unit that runs the Yjs sync server (`y-partyserver`'s `YServer`, WebSocket Hibernation) and persists the doc via its `onLoad`/`onSave` hooks into DO storage (persistence is our wiring, not automatic). The data plane: a canvas lives in the cloud whether or not a desktop app is connected. The spike syncs one DO per **workspace** — every tab's `.canvas` carries the same `docId` — because the runtime owns one `Y.Doc` per process with tabs as a data slice inside it; splitting per tab to match the ADR is recorded follow-up, not spike work. Code lives in the `server/` workspace package (`server/src/canvas-doc.ts`).
- **Device credential** — the anonymous better-auth session cookie the desktop app mints on first cloud contact and persists in `cloud-credentials.json` (mode 0600, outside `preferences.json`, never in a `.canvas` file). Every owner-scoped server call reuses it instead of re-minting a principal.
- **`specular connect`** — the headless agent CLI verb (`src/main/connect-command.ts`): redeems a share link, joins the canvas Durable Object as its own Yjs peer, and writes/updates an HTML file entity — the agent door into the HTML prototyping loop with no desktop app running.
...
- **Owner principal** — the better-auth account every cloud doc and grant hangs off, minted anonymously on the desktop app's first cloud contact (no login UI). Real sign-in later *links* an identity to the existing principal — never a data migration. The billable unit for the hosted sync tier; invisible plumbing until then.
- **Capability link** — a share URL carrying a doc id plus a **capability token** (`view` | `comment` | `edit`) for guest access with no account. The token, not the path, is the security boundary, and it is the opaque key of a **grant row** in D1 (never a stateless signed token — grants must be enumerable, revocable, and owner-attributed). One durable link per scope per canvas: Copy link is idempotent; reset rotates the token. An edit link is a write capability — treat like a password; Copy link defaults to `comment`.
- **Share popover** — the toolbar-anchored surface for sharing (ADR 0018 §4b): Copy link + scope dropdown, active-link list with reset/revoke. Dev-flagged (`debug.cloudShare`), lives behind the toolbar's cloud icon (`SharePopover.tsx`). The first Copy link is the publish moment — it attaches the canvas to the owner principal's cloud set and pushes doc + assets; an unshared canvas has zero server footprint. Links always open the view+comment web client (not yet built); `specular://` hands off to the desktop app.
- **Agent as peer** — an agent (local or cloud) joins a canvas as an ordinary authenticated Yjs peer. It edits doc data headlessly (no rendering). Agent connection tokens are first-class (narrow scope, short TTL, audit-logged, separately revocable) and issued through two doors: the owner's agents use account-scoped credentials (better-auth `apiKey`); guest agents redeem a pasted capability link for a derived token that dies with the link. Remote/agent transactions are origin-tagged (extending the `'user'` tag in `syncRuntimeToDoc`) to keep them out of the local undo stack and to attribute presence.
- **Server-readable by design** — synced content is not end-to-end encrypted: the web client, snapshot tiles, the sandbox origin, and cloud agents all require the service to read plaintext. Privacy posture is encryption at rest + owner-operated infrastructure (ADR 0018 §4).
