...
- **Page interactive (entered)** — runtime state `interactivePageId: string | null` in main (`runtime-context.ts`). Names the page the user has *entered*; only it forwards pointer input, owns keyboard, and has its content blocker lifted. A merely-**selected** page is not interactive — keyboard stays on aboveView so canvas shortcuts (Delete) act on the frame. Enter via a second click on the selected page or a double-click on its body; exit (back to selected) via Escape / click-away / selecting elsewhere. This **select-first / interact-second** model supersedes the earlier "single-selected = interactive" behavior. Broadcast as `LayoutUpdateData.interactivePageId`. See [ADR 0022](./docs/adr/0022-pages-select-first-interact-second.md).
