...
- **Page focus** — a page entered for direct interaction (`interactivePageId` in `runtime-context.ts`). When set, pointer events are forwarded to the live page via `sendInputEvent`; the gate stays open (aboveView remains visible but yields pointer events to the page). Entered via second-click or double-click on a selected page; exited via Escape / click-away. See [ADR 0001](./docs/adr/0001-click-to-enter-frame-focus.md) for the original design; that ADR's `frameFocus` / `aboveView.setVisible(false)` gate model has since been superseded — the gate no longer toggles on page entry and `frameFocus` no longer exists as a runtime field.
- **Gate** (a.k.a. **input gate**) — `aboveView.setVisible(...)` predicate. Open unless the `inspect` tool is active with `commentOverlayActive === false`. The single arbiter of who receives canvas-region pointer events. See `src/main/runtime/gate-predicate.ts`.
