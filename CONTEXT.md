...
- **Space** — a folder of canvases (Obsidian-vault analogue). The chosen folder *is* the space: `.canvas` files sit directly inside it, alongside `assets/` and a `.specular/` metadata directory. One space is open per window. Today the location is hard-coded to `userData/workspaces/default/`; [ADR 0033](./docs/adr/0033-user-chosen-space-folder.md) (proposed) makes it user-chosen via onboarding and Settings, with prompts — never guesses — for migration and for a space that is missing at boot, and unifies the vocabulary (§7): "workspace" is retired, folder-level code renames `workspace-*` → `space-*`.
- **Tab** — one canvas open in the app: `{id, name, snapshot, annotations}` in `workspaceTabs` (renaming to `spaceTabs` with ADR 0033's implementation). Internal and CLI term only — the sidebar presents tabs to the user as canvases. Exactly one tab is active, and the runtime arrays hold only that tab's state; the others are inert snapshots plus a `.canvas` file each. `specular workspace` (becoming `specular canvas`, old verb aliased) reports the active tab and every tab (id, name, entityCount) in its `appState`, so an agent can tell which canvas answered.
