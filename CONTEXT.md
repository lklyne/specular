...
- **Camera** — the `{ x, y, zoom }` viewport transform owned by main and broadcast to renderers. Canvas-space DOM (borders, entity bodies, edges, selection chrome) rides one `translate/scale` container per renderer. Native pages (`WebContentsView`s) can't ride the transform, so main follows the camera to reposition them (`setBounds`) and re-emulate on zoom-*settle*. Note: [ADR 0023](./docs/adr/0023-renderer-owned-camera-gpu-panzoom.md) explored moving camera ownership to the renderer and compositing pan/zoom on the GPU; that approach was rejected (see that ADR's postmortem).
...
- **Gate** (a.k.a. **input gate**) — `aboveView.setVisible(...)` predicate. Open whenever the `inspect` tool is not active (that is the only tool that closes the gate — see `src/main/runtime/gate-predicate.ts`). The single arbiter of who receives canvas-region pointer events. The earlier `frameFocus: { id, since } | null` runtime variable described in [ADR 0001](./docs/adr/0001-click-to-enter-frame-focus.md) was superseded by `interactivePageId` (ADR 0022); the gate no longer closes on page focus.
...
```ts
type Tool =
  | { kind: 'select' }       // default — persistent
  | { kind: 'hand' }         // persistent — pan the canvas
...
- **Variants live in tool defaults, not in the union.** `add-shape` no longer carries `shapeKind`; `draw` no longer encodes `brushType` via implicit Tool state. Both are picked from the tool-mode popup and persisted to app settings (per ADR 0009). `add-text` and `add-sticky` are separate tool kinds (per [ADR 0013](./docs/adr/0013-popup-menus-v2.md) — the earlier `add-text` union with a `style: 'plain' | 'sticky'` discriminant was removed in favour of two distinct tools).
