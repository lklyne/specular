...
- **Synced cursor** — the presence cursor sourced from interaction sync: the user's own cursor reflected onto each sync peer at that peer's resolved element. Same rendering treatment as agent cursors (filled glyph, gerund label, target halo, spline travel) with one reserved color so it reads as the user's reflection, not another actor. When a peer has no confident locator match, the synced cursor wiggles briefly at its last matched position and hides; the same wiggle marks a refused click ("can't do that here").
