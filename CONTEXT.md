...
- **Gap handle** — the drag target between a managed group's adjacent children that edits the group's persisted `layoutGap` (Figma-style). A strip spanning the gap and the full cross-axis extent of the row/column, a geometric `gap-handle` hit-test layer just below `reorder-handle` (the center dot wins where they'd overlap), painted in `aboveView` with a col-/row-resize cursor when the group or a child is selected. Managed groups only — loose selections keep arrange. The drag runs in the `resizing-gap` interaction mode: cursor movement along the packing axis maps 1:1 onto the gap (clamped ≥ 0), move ticks update only the broadcast gap (the renderer previews child positions from it — no doc writes), and commit writes once via `setGroupLayoutGap` as one undo step. Selector: `src/shared/gap-handles.ts`; coordinator: `src/main/gap-gesture.ts`.
