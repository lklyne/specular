...
- **Make auto-layout** — the persist-it upgrade for a reorderable arrangement, and the headless entry point (no toolbar yet): `Shift+Cmd+A`, `specular auto-layout <ids|groupId>`, or `POST /groups/auto-layout`. Marks a selection (or existing group) as a managed row, seeding the sequence to current left-to-right order. Reorder is reachable *before* this step on any reorderable row; making a group is how you keep the arrangement durable. The wrap-in-frame button, gap handles, direction/align/distribute controls are Milestone 2.
