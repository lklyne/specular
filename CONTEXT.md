...
How ephemeral state gets from main to the canvas renderers. The vocabulary is shared across processes — main's copy and each renderer's copy are the same shape. See [ADR 0036](./docs/adr/0036-diffed-runtime-store.md).

- **Runtime store** — the normalized form of `LayoutUpdateData` (`src/shared/runtime-store.ts`): scene entities in a map keyed by id, plus a handful of small slices. Main holds one as the baseline of what renderers are believed to have; each renderer holds its own. Not persisted, not undoable — this is the ephemeral layer, distinct from the Y.Doc.
- **Slice** — one named top-level piece of the store (`camera`, `chrome`, `scene`, `selection`, `tool`, `focus`, `hover`, `interaction`, `inspect`, `annotations`, `edges`, `fixProgress`, `presence`, `pageScroll`, `annotationBboxes`). The unit of subscription (`useSlice`) and the unit of routing: each renderer is sent only the slices it draws from, which is how `inspect` — the selected page's whole component tree, most of the payload — reaches agent-layer and nothing else.
- **Patch** — one addressed change, on the `runtimePatch` channel. Either a slice replaced wholesale or one entity in the map replaced or removed. Emitted straight from a mutator (hover, page scroll, annotation bboxes) or fallen out of the layout pass's diff.
- **Patch batch** — every patch one layout pass produced, sent together and applied together, so a renderer never paints a half-applied pass.
- **Snapshot baseline** — the full `layoutUpdate` a renderer gets on connect and again on the first pass at least a second after the last one. What makes patches safe to be lossy: a dropped or mis-applied patch heals on the next snapshot rather than leaving stale chrome. Load-bearing — never remove the cadence.
- **Drift watchdog** — the dev-gated renderer check that diffs each arriving snapshot against what the patch stream accumulated and counts disagreements (`runtime-store-drift.ts`). Catches the silent failure — a leaky subscription or a lossy producer — that patches otherwise hide. Zero drift over a session is the release gate.
- **Viewport nudge** — the deliberate exception. Pan and zoom ride their own delta channel with a renderer-side self-reconcile, because a camera move over an unchanged scene is not a scene edit.
...
- **Idle** — the app is unfocused, the blur grace has elapsed, no awake hold is outstanding, and no agent activity is in its trailing window. Idle pages are frozen (`Page.setWebLifecycleState`), never hidden: a frozen page still yields its last frame to `capturePage`, which is how agents see (`page-idle-throttle.ts`). Not CPU-throttled — see the ADR's postmortem. Inline HTML files (iframes inside canvas-bg) get the verdict as the `idle` runtime slice and hide themselves instead.
