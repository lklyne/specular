...
## 6. Where the spikes landed

Both spikes ran in one prototype window, the Offscreen Rendering Lab
(`View → Open Offscreen Rendering Lab`, `src/main/osr-lab/`,
`src/preload/osr-lab.ts`, `src/renderer/osr-lab/`), built independent of the
canvas runtime so its numbers were about Electron's offscreen path rather
than Specular's layout engine. Spike 1 (the texture tier: shared-texture vs.
bitmap-JPEG delivery, at 9/20/40 synthetic pages) found shared-texture flat
through 20 pages with the first cracks at 40; a follow-up animated-content
run reversed the mode call to shared-texture-first once concurrent page
animation was in the mix (bitmap-JPEG's synchronous per-frame encode
collapsed to ~1/8th native rate). Spike 2 (can the interactive page be
offscreen too) confirmed pointer/wheel forwarding, cursor mirroring, file
upload, and permission prompts all work, found IME composition limited to
whole-string commits, and found printing and drag-out untested at spike time
but not blocking. Both spikes' findings are recorded in
[ADR 0038](adr/0038-offscreen-texture-canvas-for-live-pages.md), which is
where the mechanism they validated was built out.

The lab's mechanism is folded into production (`src/main/runtime/page-host.ts`
and the rest of ADR 0038's Implementation section) and the lab window itself
is deleted. The benchmark for this path going forward is the app's own
`/perf/pan-zoom/run` HTTP route against the real canvas (see
`docs/perf-tracing.md`), not a standalone lab.
