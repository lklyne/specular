...
**Status:** Accepted — stack-order mutations in `src/shared/entity-order-math.ts` and `src/main/runtime/entity-order-state.ts`; `src/main/routes/stack-order.ts` exposes them over HTTP; Notes/Pages sidebar sections implemented in `src/renderer/left-sidebar/App.tsx`.
**Date:** 2026-05-11
