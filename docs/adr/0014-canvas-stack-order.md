...
5. **Sidebar drag-to-reorder.** ✅ Landed. `useDragReorder` hook in `src/renderer/left-sidebar/`; `canvas-reorder-sidebar-item` IPC channel; `api.reorderSidebarItem` + `api.reparentSidebarItems` handlers.
6. **Context-menu and keyboard shortcuts.** ✅ Landed. `stack-bring-forward` / `stack-send-backward` / `stack-bring-to-front` / `stack-send-to-back` bindings in `src/shared/bindings.ts` with `Cmd+]` / `Cmd+[` / `Cmd+Shift+]` / `Cmd+Shift+[` accelerators; context-menu wired.
7. **Edges in `entityOrder`.** ✅ Landed. Edge ids interleave in `entityOrder`; `deserializeEntityOrder` in `json-canvas-serializer.ts` accepts edge ids; new edges default to the top.
8. **Migration.** ✅ Landed. `enforceGroupContiguity` runs after deserialisation in `workspace-restore.ts`.
9. **HTTP API.** ✅ Landed. `src/main/routes/stack-order.ts` exposes the four mutation routes.
