...
**Status:** Accepted — binding registry in `src/shared/bindings.ts`, dispatcher in `src/main/runtime/binding-dispatcher.ts` / `binding-handlers.ts` / `binding-accelerator.ts`. Renderer shortcut hooks removed; app-menu accelerators derived from the same table.
**Date:** 2026-05-12
