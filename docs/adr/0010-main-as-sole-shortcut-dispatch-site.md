...
**Status:** Accepted — fully landed. `keyboard-shortcuts.ts` deleted; `binding-dispatcher.ts`, `binding-handlers.ts`, and `src/shared/bindings.ts` are the sole dispatch path. Renderer hooks deleted. App menu reads from the same `BINDINGS` table.
**Date:** 2026-05-12
