...
**Status:** Mostly implemented — §1 (palette resolver), §2 (Tool union restructure: `add-sticky` added, `add-text.style` removed), §3 (cross-kind morph — see supersession note above), §4 (Tool union), §5 (toolbar regrouping), §6 (text size), §7 (element-name composer), and §8 (page popup additions) have all landed. Deferred: shape border width in the popup (noted in §Deferred).
**Date:** 2026-05-15
