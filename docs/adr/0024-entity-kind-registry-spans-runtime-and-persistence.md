...
7. **Renderer-side per-kind capabilities live in an exhaustive shared table**, not the main-process registry. `Record<CanvasEntityKind, { hasAnchors, minSize, aspectMode }>` in `src/shared/entity-kind-caps.ts`, consumed by hit-test and the pointer router (layer rules forbid renderer → main imports). TypeScript exhaustiveness makes a new kind a compile error there — forced decision, not silent default. (Note: `hasChrome` was in the original design but removed by [ADR 0028](./0028-retire-chrome-header-slot-model.md); it is absent from the current caps table.)
