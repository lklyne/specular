...
7. **Renderer-side per-kind capabilities live in an exhaustive shared table**, not the main-process registry. `Record<CanvasEntityKind, { hasChrome, hasAnchors, minSize, aspectMode }>` in `src/shared/`, consumed by hit-test and the pointer router (layer rules forbid renderer → main imports). TypeScript exhaustiveness makes a new kind a compile error there — forced decision, not silent default. **Note:** `hasChrome` was subsequently removed by [ADR 0028](./0028-retire-chrome-header-slot-model.md) (2026-07-07). The current table at `src/shared/entity-kind-caps.ts` is `{ hasAnchors, minSize, aspectMode }`.
