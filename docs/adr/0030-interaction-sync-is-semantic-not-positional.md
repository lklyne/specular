...
- **Confident-or-skip, one policy for cursor and click.** A peer acts only on an unambiguous match (exact unique identity key, or a clear top score over the runner-up). Anything less: the synced cursor wiggles at its last matched position and the event is dropped on that peer. The mirror tells the truth or stays silent — it never guesses, and the cursor never hovers an element the click would refuse.
...
- **Recovery is the existing reload.** Divergence is inherent to best-effort mirroring (a skipped event forks peer UI state). Nav sync already propagates reload across the set, so reload-on-source is the resync affordance; no new UI.
