...
**Status:** Proposed — spike validated the perf hypothesis; production build-out (agent routes, capability parity, pool tuning) is unstarted.
...
2. **Spike 2's interactivity checklist, updated:**
   - Confirmed working (prior lab sessions): popup widgets (`<select>`, own texture), cursor mirroring, `capturePage()`, DevTools, hover/scroll/click/drag-select.
   ...
   - Corrected finding: **`Page.printToPDF` is not implemented in Electron's CDP debugger domain** (`-32601 'Page.printToPDF' wasn't found`) — printing must be spiked through the main-process `webContents.printToPDF()` API directly, not over remote CDP. Still untested.
   ...
   - Still untested, deferred deliberately: dragging content *out* of a page, and printing. Neither is a regression risk worth blocking on: drag-out is already a documented soft carve-out in the *current* shipped architecture (`docs/plans/aboveview-interactive-layer.md:245-246` — "Scenario 14 (drag-out) works. If not, document the workaround" — it's tested informally today and allowed to fail), and printing has never existed as a feature anywhere in the app (`printToPDF`/`window.print`/a print menu item — zero hits). Both stay open items to close before claiming full parity, but neither gates starting production build-out.
...
**Costs / open work:**
- Agent HTTP route parity (DOM snapshot, selector queries, CDP proxy) needs building for offscreen pages — currently zero.
- Spike 2's remaining checklist items (drag-out, print) need running before claiming full interaction parity, though neither is a regression risk (see Open questions).
...
- `docs/interaction-layer.md` needs a full rewrite if this graduates — its three-plane model and layering invariants (§6) are the thing being removed, not preserved.

**Not resolved by this ADR:** whether shape B fully replaces shape C, or lands as a hybrid. That is the subject of closing open question 2.
...
- **Interaction with ADR 0035's idle-freeze policy** — confirm `setWebLifecycleState('frozen')` and OSR's own viewport culling (`stopPaintingOffscreen`) compose correctly rather than fighting or duplicating each other.
