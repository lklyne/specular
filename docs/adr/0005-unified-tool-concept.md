...
**Later divergence (ADR 0013):** `add-text; style: 'plain' | 'sticky'` was split into two separate top-level tool kinds: `{ kind: 'add-text' }` (plain) and `{ kind: 'add-sticky' }` (sticky). The union no longer carries a `style` field on either. A `{ kind: 'hand' }` panning tool was also added after this ADR.
**Date:** 2026-05-08
