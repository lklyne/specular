...
**Status:** Accepted — landed alongside ADR 0010 (Keyboard binding registry, #76). `firesFromPageFocus: boolean` field added to the `Binding` type in `src/shared/bindings.ts`; the dispatcher skips any binding that lacks this flag when `pageFocusActive` is true, so the page (not the app) receives the keystroke.
**Date:** 2026-05-12
