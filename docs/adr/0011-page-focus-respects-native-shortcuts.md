...
**Status:** Accepted — landed alongside ADR 0010. `src/shared/bindings.ts` suppresses all bindings when `pageFocusActive` is true unless the binding carries `firesFromPageFocus: true` (escape-page-focus and a handful of overlay bindings that must work mid-page interaction). The old per-view `handleShortcuts: false` flag is gone.
**Date:** 2026-05-12
