...
The router consumes the full action set (`FULL_ROUTER_CONSUME`): `enter-frame-focus`, `begin-entity-drag`, `begin-group-drag`, `begin-resize`, `begin-edge-drag`, `toggle-select`, `background-click`, `begin-marquee`, `begin-pan`. The remaining viewport helper is limited to wheel zoom and middle-button pan — plain wheel/two-finger scroll does **not** pan the canvas (ADR 0017); it scrolls a hovered page or note, or does nothing over empty canvas. Pan is reachable via the hand tool, space-drag, and middle-button drag.
