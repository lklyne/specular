...
- [0010 — Main is the sole shortcut dispatch site](adr/0010-main-as-sole-shortcut-dispatch-site.md) — consolidates keyboard shortcut handling into main; `binding-dispatcher.ts` and `bindings.ts` are the landing.
- [0011 — Page focus respects native shortcuts](adr/0011-page-focus-respects-native-shortcuts.md) — a keyboard-focused page receives native shortcuts (Cmd+Z etc.); only `escape-page-focus` and `reset-viewport` fire while focused.
...
- [0013 — Popup menus v2: palette, text size, cross-kind morph, toolbar regrouping](adr/0013-popup-menus-v2.md) — locks down popup visual design; §3 cross-kind morph was superseded and deleted (2026-07); see the ADR postmortem for what shipped vs. what was cut.
- [0014 — Canvas stack order and the Notes/Pages sidebar](adr/0014-canvas-stack-order.md) — flat `entityOrder` + group-contiguity invariant defines front-to-back stacking; sidebar splits into Notes/Pages sections. Slices 1–4 landed; slices 5–9 still pending.
- [0015 — Auto-layout groups](adr/0015-auto-layout-groups.md) — Figma-style auto-layout: selected group packs children into row/grid with reorder-on-drag. Milestone 1 (row + reorder) and gap handles landed; floating toolbar and column/grid pending.
