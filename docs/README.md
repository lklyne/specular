...
- [0010 — Main is the sole shortcut dispatch site](adr/0010-main-as-sole-shortcut-dispatch-site.md) — consolidates keyboard shortcut handling into main via `src/shared/bindings.ts` + `src/main/runtime/binding-dispatcher.ts`.
- [0011 — Page focus respects native shortcuts](adr/0011-page-focus-respects-native-shortcuts.md) — a keyboard-focused page sees native shortcuts; only `escape-page-focus` and `reset-viewport` fire while page-focused.
...
- [0013 — Popup menus v2: palette, text size, toolbar regrouping](adr/0013-popup-menus-v2.md) — eight-slot color palette, `add-sticky` as a separate tool, `textSize` defaults, and popup visual design; Phase 6 (wireframe mode) was deleted post-acceptance.
- [0014 — Canvas stack order and the Notes/Pages sidebar](adr/0014-canvas-stack-order.md) — flat `entityOrder` + group-contiguity invariant defines front-to-back stacking; sidebar splits into Notes/Pages sections; slices 1–4 landed, sidebar drag/shortcuts/edge-in-order remain.
- [0015 — Auto-layout groups](adr/0015-auto-layout-groups.md) — Figma-style auto-layout: managed groups pack children into row/column with reorder-on-drag and gap handles; `grid` and floating toolbar deferred.
