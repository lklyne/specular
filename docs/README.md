...
- [0010 — Main is the sole shortcut dispatch site](adr/0010-main-as-sole-shortcut-dispatch-site.md) — keyboard shortcut handling consolidated into `src/shared/bindings.ts` + `binding-dispatcher.ts`; `keyboard-shortcuts.ts` deleted (landed #76).
- [0011 — Page focus respects native shortcuts](adr/0011-page-focus-respects-native-shortcuts.md) — a keyboard-focused page sees native shortcuts; bindings must opt in via `firesFromPageFocus` to fire during page focus (landed #76 alongside 0010).
...
- [0013 — Popup menus v2: palette, text size, cross-kind morph, toolbar regrouping](adr/0013-popup-menus-v2.md) — locks down popup visual design: 8 swatches, per-entity size, cross-kind morph, plain/sticky/markdown split (landed #121; Phase 6 wireframe popup deleted before it shipped).
- [0014 — Canvas stack order and the Notes/Pages sidebar](adr/0014-canvas-stack-order.md) — flat `entityOrder` + group-contiguity invariant defines front-to-back stacking; stack-order context menus and `Cmd+[]/]` shortcuts landed (landed #166).
- [0015 — Auto-layout groups](adr/0015-auto-layout-groups.md) — Figma-style auto-layout: selected group packs children into row/grid with reorder-on-drag and draggable gap handles (landed #170, #311).
