...
- [0010 — Main is the sole shortcut dispatch site](adr/0010-main-as-sole-shortcut-dispatch-site.md) — single `before-input-event` dispatcher in main calls `dispatchKey()` against the shared `BINDINGS` table; renderer shortcuts fire one IPC; `keyboard-shortcuts.ts` and renderer shortcut hooks are gone.
- [0011 — Page focus respects native shortcuts](adr/0011-page-focus-respects-native-shortcuts.md) — all bindings fall through to a focused page unless they explicitly set `firesFromPageFocus: true`; escape and reset-viewport opt in; implemented alongside ADR 0010.
...
- [0013 — Popup menus v2: palette, text size, cross-kind morph, toolbar regrouping](adr/0013-popup-menus-v2.md) — 8-slot palette (`canvas-colors.ts`), text size dropdown, add-sticky, device-frame popup controls; cross-kind morph (§3) was deleted; `add-document` retained as independent tool (see inline 2026-07 notes).
- [0014 — Canvas stack order and the Notes/Pages sidebar](adr/0014-canvas-stack-order.md) — flat `entityOrder` + group-contiguity invariant defines front-to-back stacking; sidebar splits into Notes/Pages sections; all 9 migration slices landed.
- [0015 — Auto-layout groups](adr/0015-auto-layout-groups.md) — Figma-style auto-layout: managed group packs children into row/column with reorder-on-drag and gap handles; grid/align/floating toolbar deferred.
