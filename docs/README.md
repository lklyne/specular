...
- [0010 — Main is the sole shortcut dispatch site](adr/0010-main-as-sole-shortcut-dispatch-site.md) — consolidates keyboard shortcut handling into main via `src/shared/bindings.ts` + `binding-dispatcher.ts`; renderer shortcut hooks deleted.
- [0011 — Page focus respects native shortcuts](adr/0011-page-focus-respects-native-shortcuts.md) — a keyboard-focused page sees all keystrokes natively except bindings with `firesFromPageFocus: true` (Escape and Cmd+1).
...
- [0013 — Popup menus v2: palette, text size, cross-kind morph, toolbar regrouping](adr/0013-popup-menus-v2.md) — **Partially adopted** — eight-slot color palette, per-entity text size, `add-sticky` as a first-class tool, and visual lock shipped; §3 cross-kind morph was retired and `add-document` was not removed (see note in the ADR).
- [0014 — Canvas stack order and the Notes/Pages sidebar](adr/0014-canvas-stack-order.md) — **Partially accepted** — flat `entityOrder` + group-contiguity invariant; Notes/Pages sidebar sections live; slices 5–9 (drag-reorder, shortcuts, edges in order, migration, HTTP API) pending.
- [0015 — Auto-layout groups](adr/0015-auto-layout-groups.md) — Figma-style auto-layout: managed groups reflow children as rows; gap handles and column mode landed; `'grid'` mode and align deferred.
