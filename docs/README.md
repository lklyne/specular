...
- [0010 — Main is the sole shortcut dispatch site](adr/0010-main-as-sole-shortcut-dispatch-site.md) — consolidates keyboard shortcut handling into a single binding registry in main; `binding-dispatcher.ts` is the sole dispatch site.
- [0011 — Page focus respects native shortcuts](adr/0011-page-focus-respects-native-shortcuts.md) — a keyboard-focused page sees native shortcuts (Cmd+Z etc.) before app-level handlers; implemented via `firesFromPageFocus` on each binding.
...
- [0013 — Popup menus v2: palette, text size, cross-kind morph, toolbar regrouping](adr/0013-popup-menus-v2.md) — **Proposed (partially shipped)** — locks down popup visual design decisions left soft by ADR 0008/0009; note: §3 cross-kind morph superseded inline; wireframe phases non-applicable (wireframe mode deleted).
- [0014 — Canvas stack order and the Notes/Pages sidebar](adr/0014-canvas-stack-order.md) — flat `entityOrder` + group-contiguity invariant defines front-to-back stacking; Bring forward/Send backward shipped in v0.5.0.
- [0015 — Auto-layout groups](adr/0015-auto-layout-groups.md) — Figma-style auto-layout: selected group packs children into row/grid with reorder-on-drag; shipped v0.3.1/v0.4.0.
...
- [0017 — Scroll does not pan the canvas in browser mode](adr/0017-scroll-does-not-pan-the-canvas.md) — wheel/trackpad scroll pans the canvas; page and note scroll forward into their content. Note: browser mode was deleted by ADR 0020; see amendment in the file.
...
- [0019 — Canvas as a document: verb-primary CLI over one entity-kind registry](adr/0019-canvas-as-document-cli.md) — CLI surface becomes verb-first (`add`/`update`/`delete`/`arrange`) dispatched through one `CanvasEntityKind` registry. `specular workspace` renamed to `specular canvas` (ADR 0033); all 4 adoption slices landed.
