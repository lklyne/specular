...
- [0006 — Unified comment tool (subsumes region-select)](adr/0006-unified-comment-tool.md) — one comment tool handles element/point clicks and region drags; amended by 0031 for page-anchored regions.
...
- [0010 — Main is the sole shortcut dispatch site](adr/0010-main-as-sole-shortcut-dispatch-site.md) — consolidates all keyboard shortcut handling into one `BINDINGS` table in `src/shared/bindings.ts` dispatched from main's `before-input-event` listener.
- [0011 — Page focus respects native shortcuts](adr/0011-page-focus-respects-native-shortcuts.md) — when a page has keyboard focus, all shortcuts pass natively to the page except `escape-page-focus` (Escape) and `reset-viewport` (Cmd+1).
...
- [0017 — Scroll does not pan the canvas in browser mode](adr/0017-scroll-does-not-pan-the-canvas.md) — **superseded in context by ADR 0020** — the browser-mode problem this solved is gone; wheel-pan is canvas-navigation only and page scroll is native.
