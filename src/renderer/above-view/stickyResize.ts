/**
 * Sticky resize policy — what a handle drag means for a sticky note.
 *
 * A sticky stores one geometry axis: width. Its height is a content-sized
 * bound (CONTEXT.md) whose sole writer is the measurement in StickyBodyLayer,
 * so every patch here drops `height`. A second writer would let the card and
 ...
 *     rewraps, so a wider note usually ends up shorter.
 ...
 *     Aspect is locked for those handles so a vertical drag drives width too.
 */

import type { ResizeHandle } from '../../shared/resize-accumulator'
import { clampTextSize } from './TextSizeDropdown'

export type StickyResizeStart = { width: number; textSize: number }

type BoundsPatch = { width: number; height: number; canvasX?: number; canvasY?: number }

export function stickyResizePatch(
  handle: ResizeHandle,
  start: StickyResizeStart,
  patch: BoundsPatch,
